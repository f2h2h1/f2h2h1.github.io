...
import { Application } from './index2.js';
import { marked } from './static/marked.esm.js';
// import path from 'commonjs-package';
// import fs from 'commonjs-package';
// import child_process from 'commonjs-package';
...
*/
    ...
    // var createPage = function(articleList) {
        // 读取模板 读取文章列表 读取文章md 生成首页列表 生成侧边列表 生成友链 生成文章html 在文章中插入其它的信息
        // fs.readFileSync('index.html', {encoding:'utf8', flag:'r'});

        // console.log(articleList[2]['html']);
    // }
    // createPage(articleList);
    ...
    for (let i = 0, len = articleList.length; i < len; i++) {
        ...
        // articleList[i]['html'] = marked.parse(articleList[i]['md']);
    }
    var application = new Application({});
    application.setTemplate(fs.readFileSync('index.html', {encoding:'utf8', flag:'r'}));
    application.setArticleList(articleList);
    application.setLinkExchangeList(exchangeList);

    let pageHtml = application.buildPage(articleList[4]);
