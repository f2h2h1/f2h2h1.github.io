...
数学
    初等数学
        ...
        几何
            ...
            三角学
                ...
                射影定理
                几个特殊的角度
                    180 120 90 60 45 30 0
                可以通过 解三角形 求出其它边的长度
                    但 解三角形 也是通过三角函数来实现的。。。
                    但 解三角形 却可以解任意角度的三角形，不单是直角三角形
                至少知道
                    一条边长度 就是标准圆的半径
                    两个角度 一个是直角 另一个是θ
                知道两个角度后，就能知道第三个角度
                所以问题就变成了，知道一边长度加三个角度如何求另外两边长度

                θ 被称为 正角 ，就是标准圆的半径和x轴的夹角，第三个角被称为 余角 
                正角对着的弦的长度就叫做 正弦
                余角对着的弦的长度就叫做 余弦
                正角对着的切线的长度就叫做 正切
                余角对着的切线的长度就叫做 余切

                英语 "Angle" 的意思是 角，"Side" 的意思是 边
                切线 与曲线相交于一点的直线
                割线 与曲线相交于两点的直线
                弦   连接曲线两点的线段
                ...
                弧
                在标准圆中
                正切 正角的 对边 比 邻边 y/x y/x
                余切 余角的 对边 比 邻边 x/y x/y
                正弦 正角的 对边 比 斜边 y/r y/1 y
                余弦 余角的 对边 比 斜边 x/r x/1 x
                正割 正角的 斜边 比 邻边 r/x 1/x
                余割 余角的 斜边 比 邻边 r/y 1/y

                三角函数有很多，但 sin、cos和tan 已经够用了
                sin、cos和tan分别是三角函数的缩写，代表以下含义：
                sin: 正弦（sine）
                cos: 余弦（cosine）
                tan: 正切（tangent）

                三角函数一般用于计算三角形中未知长度的边和未知的角度，在导航、工程学、物理学、天文学方面都有广泛的用途

        概率论和统计学
            随机变量
            数学期望
        暂时不会分类的
            整式 单项式 多项式 项数和次数 近似数 精度 分式 倒数 绝对值
            集合
            逻辑
            复数
            不等式
    ...
    离散数学
        ...
        理论计算机科学
            自动机
        拓扑学
        运筹学（Operations Research，又称运营研究）
            单一决策者
            目标 最优解（optimum）
        博弈论
            多个决策者
    ...
    数学的历史
        三次危机
            无理数的出现
                第一次数学危机中，因为发现腰长为1的等腰直角三角形的斜边长度无法写成有理数，从而引申出日后的无理数概念。
            无穷小是什么
                第二次数学危机得解决微积分引入无穷小量而产生的问题。
            集合论中的悖论
                第三次数学危机则是因罗素悖论而起，它点出朴素集合论中的缺失。
        微积分之前
        微积分之后
    黄申
        智能聊天机器人：核心技术与算法
...
数据分析/机器学习/数据挖掘/数据科学/大数据
    ...
    可能用到的语言或工具
        ...
        Octave MATLAB Mathematica SAS SPSS stata
        Excel Access
        PowerQuery
...
人工智能
    ...
    神经网络（Neural Network， NN）
        人工神经网络（Artificial Neural Network， ANN） 简称 神经网络（Neural Network， NN）
        神经网络在多数情况下可以归类到 经典的机器学习
        但神经网络是深度学习的基础，值得特意学习
        多数情况下 神经网络算是 有监督学习
        多数情况下 神经网络算是 既可以用于分类也可以用于回归
        通常认为，拥有 3 层或更多隐藏层（Hidden Layers） 的神经网络就可以被称为深度神经网络。
            浅层网络（Shallow Network）： 通常指只有 1 到 2 个隐藏层的网络（例如传统的多层感知机 MLP）。
            深度网络（Deep Network）： 通常指拥有 3 个及以上隐藏层的网络。
    深度学习（Deep Learning）
        深度学习 就是指 深度神经网络（Deep Neural Network, DNN）
        早期的神经网络 3层以上就算是多层的了
        2012 年（AlexNet）： AlexNet 有 8 层
        现在的神经网络 几十上百层都很常见
        现在的语境下，神经网络默认都是深度神经网络
        前馈神经网络（Feed Forward Network, FNN）
        卷积神经网络（Convolutional Neural Networks, CNN） 主要用在 计算机视觉
        循环神经网络（recurrent neural networks，RNN）      主要用在 自然语言处理
        Transformer （直译为“变换器”） 也算是一种神经网络
    大语言模型（Large Language Model, LLM）
        ...
        AI Agent/bot/Generative AI/上下文窗口/微调/RAG/MOE
            ...
            温度（Temperature）
                ...
                这是 deepseek 对 temperature 参数设置的建议
            ...
            Function Call
                tools
                最早支持 function call 的是 gpt-4-0613和gpt-3.5-turbo-0613
            接口参数
                max_token/max_completion_tokens
                Temperature
                top_p
        ...
        模态（modal）
            ...
            全模态（Omni modal ）
                ...
                全模态至少要同时支持 文本 图片 音频 的输入
                Omni源自拉丁语“omnis”，意为“全部、一切”，常用于表示全能、全面或多功能的概念
                    在英语中，它常用于构成表示全面或全能的词汇，
                    Omnipotent（全能的）、Omniscient（全知的）
        局限性
            幻觉
            偏差
    ...
    著名的公司
        ...
        国内的
            幻方量化/深度求索 百度 阿里 腾讯 字节 华为 科大讯飞 太多了，几乎所有公司都有大模型
            四小龙/CV四小龙
                商汤、旷视、云从、依图
            六小龙
                四小强 + 百川智能 、 零一万物
                四小强 + deepseek 、 面壁智能(modelbest)
            四小强
                月之暗面 智谱（清华系） 稀宇科技 阶跃星辰
            五虎？
                四小龙 + ？
                模型五虎？
                    六小龙中排列组合出五个
            清华系
                月之暗面 智谱 面壁智能 腾讯的混元 都有清华的背景
    热门的大模型
        gpt
        Claude
        Grok
        Gemini
        gemma 谷歌的，Gemma是谷歌基于Gemini技术打造的轻量级模型系列，也是开源的
        LLaMA mate llama系列似乎被抛弃了
        Muse Spark mate新的模型，但闭源
        Phi 微软的
        BLOOM 来自法国的，也是开源的
        Mistral 来自法国的，大部分模型开源，也有小部分闭源
        Granite IBM 小尺寸开源，大尺寸闭源
        Nemotron NVIDIA
        lfm Liquid Liquid 是 mit csail（计算机科学人工智能实验室） 的一个项目
        deepseek 深度求索
        通义千问
        文心一言
        豆包 字节 豆包居然是闭源的
    ...
    大模型的排行榜
        ...
        https://lmarena.ai/leaderboard https://arena.ai/zh/leaderboard
        ...
        金融相关
        https://github.com/jeinlee1991/chinese-llm-benchmark/blob/main/leaderboard/金融.md
        https://nonelinear.com/static/benchmarking/finance.html
        https://cnfinbench.opencompass.org.cn/dataset
    应用
        文本
            生成文本 对话
        视频
        ...
        AI代码生成
            cursor
                商业应用，根据不同的模型收费，可以用 openai 和 Anthropic 的模型
            Trae 也是来自字节
            MarsCode (豆包)
            文心快码
            GitHub Copilot
            ...
            OpenCode https://github.com/anomalyco/opencode
                开源的，需要自行接入大模型
                OMO（On My OpenCode） OMO 是 OpenCode 的增强插件，
                    提供多智能体协作、LSP 集成、自动化工作流等功能。所有功能默认启用，开箱即用。
            kilo code
            cline
            AI 编程工具一般有三种产品形态
                IDE 插件
                ...
                终端。还有一些公司，专门做基于终端（也就是命令行）的工具，
                    ...
                    命令行工具还可以用在 ci/cd 里
                cli(Non-interactive Interactive TUI) web gui IDE插件
            AI 编程工具
                可以使用专用的编程模型，例如 GPT-5.2-Codex
                也可以使用通用的模型，例如 DeepSeek-V3.2
            vscode中的插件
                Roo Code https://github.com/RooCodeInc/Roo-Code roocode停更了，这个是fork https://github.com/Zoo-Code-Org/Zoo-Code/
                continue https://github.com/continuedev/continue
                GitHub Copilot https://github.com/microsoft/vscode-copilot-chat
            似乎主要的模型供应商都有代码相关的模型或产品。。。
    有影响力的人
        AI 三巨头
    ...
    相关的框架
        ...
        Apache Spark MLlib
    相关的书籍和仓库
        面向初学者的数据科学课程 https://github.com/microsoft/Data-Science-For-Beginners
        ...
        机器学习系统教程：AI 工程原理与实践
            https://jimmysong.io/zh/book/ml-systems/
        智能体设计模式：智能系统构建实战指南 
            https://jimmysong.io/zh/book/agentic-design-patterns/
        智能体构建指南：AI 原生基础设施与运行时架构实践
            https://jimmysong.io/zh/book/ai-handbook/
    Kaggle
    ...
    ollama
        ...
        openai api
            ...
            role 可选："system" (系统设定), "user" (用户输入), "assistant" (助手回复)
        前端
            openwebui
            https://github.com/ollama-ui/ollama-ui
            windows 的ollama自带一个简易的ui
财政（Fiscal）、经济（Economics）、金融（Finance）、股票、基金、量化（Quantitative）
    经济（Economics）
        ...
        微观经济学（Microeconomics）
            研究微观经济体系中基本单元（个体、企业）的经济行为。
            微观经济学重视需求与供给如何影响个人、达成交易，并形成市场中的均衡价格。
            产业
                第一 原料开采和生产 农业/采矿业
                第二 商品制造 工业
                第三 服务业
    财政
        Fiscal 狭义的财政
        Public finance 广义的财政 公共财政
            财政（Fiscal） 财政部
                税收
                国债
            货币（Monetary） 中央银行 美国的是美联储
                利率
                存款准备金
                存款准备金比例
                黄金储备
            贸易（Trade） 贸易部/商务部
                关税
    交易（trade） 贷款（Loan）储蓄（saving）
    汇兑（Exchange）就是汇款
    理财（WMPs）理财其实是中国特色的金融产品
        Wealth Management Products
        财富 管理 产品
    保险（Insurance） 外汇（Foreign exchange 简写为 Forex 或 FX） 期货（futures） 基金（funds） 信托（Trust） 家族办公室（Family Office）
    REITs（不动产信托）
        ...
        Investment n. 投资
        Trust n. 信托
        REIT是先向投资人募集资金，再用以投资不动产。
        REIT具有所有权，平时收取租金并定期分配现金股利给投资人，若处分不动产则收益或损失都归投资人。
        形式和基金差不多，只是底层资产是 不动产
    基础资产（Underlying Asset）
        现金（Cash） 存款（deposit） 股票（stocks） 债券（bonds） 保险 商品（Commodity）
            外汇 期货 基金 都不是基础资产
            多数情况下 不动产（Real estate） 也可以算作基础资产
    证券（Security）
        不是实物商品，而是一种法律承认的、可流通的权利凭证
        凭据证券
            借条、收据、等
    ...
    基金（funds）
        按 投资标的 划分
            货币基金（Money funds, Money market funds， Money market mutual funds）
            债券基金（bond fund）
                主要投资债券
            股票基金（stock fund）
                主要投资股票
            平衡型基金（Balanced mutual fund），也被称为混合基金
                股票 债券 存款都有
            商品基金（Commodity Fund）
                以大宗商品为主要投资标的的基金品种，投资标的包括但不限于：
                大宗商品上市公司、大宗商品现货及期货等，常见的大宗商品种类有：黄金、白银等贵金属、原油和农产品等。
            REITs（不动产信托）
            FOF（Fund of Funds）——基金中的基金
                主要投资其它基金
            宽基基金
                跟踪宽基指数的基金
                "宽基"是指指数的选样范围较"宽"，不限于特定行业或投资主题，具有较强市场代表性的一类指数
                基 是 基准 的意思？
                “宽基”代表该指数的成分股选样空间“宽”，不局限于特定行业或投资主题，具有广泛的市场代表性
                相对地 有 窄基指数
                    重点关注某个特定领域
                    例子
                    宽基指数：沪深300、中证500、中证A500、上证50、创业板指等
                    窄基指数：医药指数、光伏指数、人工智能主题指数等
        按 交易机制 划分
            ...
            Open-End fund 开放式基金
                可以在随时买入卖出的基金，这个概念相对于 封闭式基金
                封闭式基金 和 开放式基金 就是一般语境下的基金
        按 募集方式 划分
            公募
        ...
        基金的费用
            ...
            托管费     交给银行的
            销售服务费 交给基金公司的
            申购费     买入时给付，交给基金公司的，只在买入时收取一次
            赎回费     赎回时给付，交给基金公司的，随持有时间增长而降低或免除
            运作费
        ...
        一个基金通常会分为 A类和C类
            A类 不收取 销售服务费 但收  申购费
            C类 不收 申购费       但收 销售服务费
            多数情况下长期持有选A类，短期持有选C类
                销售服务费 是按日计提，虽然我不知道怎么算的，但长期持有，费用肯定高过A类的 申购费
            A类和C类基金的代码不一样，它们是同一只基金的两种不同份额类别，因此有独立的基金代码。
            其实还有B类的，但没什么存在感，
    金融衍生品/金融衍生工具
        Derivative（衍生品）
        港澳称金融衍生产品
        ...
        四大类衍生品
            ...
            期货（futures）
                ...
                商品期货
                    能源: 原油 天然气 液化石油气 煤
                    金属: 黄金、白银、铜、铝、铅、锌、镍、钯、钴
                    农产品: 玉米、小麦、大豆、大豆油、大豆粕、棉花、糖、咖啡、可可、棕榈油、菜籽油、菜籽粕、豆油
                    软商品: 木材 橡胶 纸浆
                    化工：聚乙烯 聚氯乙烯 聚丙烯 乙二醇苯 乙烯 纯苯 甲醇 硅铁 锰硅 尿素 纯碱 短纤 对二甲苯 烧碱 瓶片 丙烯
                金融期货
                    外汇类期货 利率类期货 股票类期货
                非传统期货
                    天气 电影票房 房地产指数
                主连
                    连续合约
                    主力连续合约
                    不是一份真实的、可以交易的期货合约，而是由该品种不同时期的主力合约的价格数据拼接而成的一个连续的时间序列数据
                    主连的作用是提供连续的价格走势图
            期权（Options）
                看涨期权/买入期权（Call Option）
                看跌期权/卖出期权（Put Option）
                末日期权
                    End of Day Options / 0DTE Options
                    是指即将到期或当天到期的期权合约。
                    具体定义有广义和狭义之分：
                        狭义：特指当天到期的期权合约（0 Days To Expiration，简称0DTE）
                        广义：指15天以内到期的期权合约，尤其是最后3-5个交易日的期权
                    之所以被称为"末日轮"，是因为越是临近到期日，
                    如果标的资产发生较大波动，期权合约价格的变化幅度可能达到几十倍甚至上百倍
                    归零是常态：买方大部分时候会亏掉全部投入的资金。
            交换（Swaps）
                利率交换
                总收益交换
                信用违约交换
    ...
    外汇
        ...
        外汇 大致有两种
            ...
            实盘外汇买卖
                没有杠杆的交易
                在中国大陆，个人参与外汇市场唯一完全合规的方式是通过境内银行办理实盘外汇买卖
                除此之外也可以通过 QDII 间接参与 外汇市场
                或使用年度便利化额度进行真实需求的购汇/结汇
        一般语境下的 炒外汇 就是指 外汇保证金交易
    轮证
        轮证 也是一种金融衍生品
            Warrants & CBBCs （更通用的名称）
            Listed Structured Products （上市结构性产品 港交所的名称）
            Wheel Certificates（中文常称为“轮式认购证”或“轮式证书”）
        "轮证"实际是"窝轮 + 牛熊证"的合称，其中：
            "轮" = 窝轮（Warrant）
            "证" = 牛熊证（CBBCs）
        轮证 主要有三种
            认股证 (Derivative Warrants)
                认股证，又称衍生权证（Derivative Warrants）或权证（Warrants）
                在香港金融市场，也音译为窝轮（Warrant）
            界内证 (CBBCs , Callable Bull-Bear Contracts)
            牛熊证 (Inline Warrants)
        轮证 在 香港 台湾 欧洲 东南亚 日韩 都有， 唯独 中国大陆 和 美国 没有
    现代意义的银行大概在14世纪出现
        存款 汇款 贷款 换汇（currency exchange） 保险箱  贴现（Discounting） 保理（Factoring）
        代销（保险，基金，理财，实物黄金，各种纪念币纪念邮票纪念品）
    股份制公司大概在16世纪出现
    板块/行业/题材/概念
        英文语境下
            板块 Sector
            ...
            题材 Theme
            概念 Concept
            美股 是 按照 ICB 或 GICS 或 BICS 分类的
                ICB
                    Industry Classification Benchmark
                    富时罗素(FTSE Russell)	全球上市公司	4级：行业、超行业、行业、子行业。与GICS竞争，逻辑略有不同。
                GICS
                    Global Industry Classification Standard
                    MSCI（明晟）与标普道琼斯指数(S&P Dow Jones Indices)	全球上市公司	4级：板块(Sector)、行业组、行业、子行业。
                BICS
                    Bloomberg Industry Classification Standard
                    彭博(Bloomberg)	全球上市公司	3级：板块、行业组、行业。与彭博数据深度集成。
            分为四个层级
                板块 Sector
                行业组 Industry Group
                行业 Industry
                子行业 Sub-Industry
            一个股票通常只有属于一个 行业 ，虽然不同的标准可能会有不同的分类
            题材 和 概念 是同义词
            题材 来自 Thematic Investing （主题投资）
                Thematic Investing 是一种投资策略
                    基于未来趋势、政策、故事的投资
        中文语境下
            板块 是股票的集合
                行业板块 大致 对应 GICS 的 Sector
                    上证 深证 都有类似的分类，按主营业务来分类
                指数板块
                地域板块
                题材 和 概念 是同义词
                xx题材/xx概念 有时也会称为 xx板块
    角色
        政府
            中央银行
            财政部
    ...
    各种黑话，各种名词
        ...
        F10
            Financial 10
            财务10
            F10数据是指在金融投资中用于查询上市公司基本面信息的综合性报告
        风格
            成长
                主要收益来自股价的增长
            价值
        ...
        头寸
            ...
            在证券
                Position 部位（持仓、头寸）
                Long Position 做多部位 做多持仓 做多头寸 多头
                Short Position 做空部位 做空持仓 做空头寸 空头
                Net Position 净部位 净持仓 净头寸
                部位（持仓、头寸）的三大属性
                    一是方向（position direction），是做多还是做空；
                    二是规模或大小（position size），头寸中的“寸”即是此意；
                    三是建立时的价格位置（price level），部位中的“位”即是此意。
        揸 买入/沽 卖出/沽清 清仓
        蓝筹/红筹/紫筹
        HALO
            Heavy Assets, Low Obsolescence
            重 资产，低 淘汰率
            “HALO交易是2026年华尔街最新流行的投资策略，由投资机构Ritholtz的CEO Josh Brown于2026年2月初首次提出，
            随后被高盛、摩根士丹利等顶级机构快速采纳并推向市场”。
            “HALO”指的是不容易被替代的重资产公司，
            主要偏向于公用事业、能源、基建、材料及工业设备等重资产行业。
            然后美国市场的分析师发现，美国市场的这类公司自去年四季度以来，表现显著优于科技股，因此这段时间的行情就被定义为“HALO交易”。
            HALO资产覆盖多个行业，主要包括：
                周期资源：矿产、有色金属、石油、煤炭（如中国神华、陕西煤业、紫金矿业）。 
                公用事业：电力、水务、核电及电网设备（如长江电力、国电南瑞）。 
                工业制造：工程机械、农机、半导体设备（如三一重工、中微公司、北方华创）。 
                交通运输：铁路、高速公路、港口、机场（如京沪高铁、招商港口）。 
                通信基建：运营商、IDC数据中心、光纤光缆（如中国移动、数据港、长飞光纤）。 
                传统能源：油气开采、燃气管道（如中国海油、新奥股份）。 
                在芯片产业中，半导体设备和晶圆制造环节也属于典型HALO资产，因其资本开支巨大、折旧周期长、客户粘性高，技术迭代慢。
        KYC
            Know Your Customer
            了解你的客户
            KYC是金融机构和其他相关机构在建立业务关系或处理特定交易前，对客户身份进行验证的一系列流程。
            KYC认证旨在防止金融犯罪，如洗钱、恐怖融资和欺诈行为，确保交易的合法性和安全性。
            KYC1认证：基础身份验证
                真实姓名
                性别
                国籍
            ...
            KYC2认证：增强身份核验
                除KYC1所要求的内容外，用户还需：
                符合要求的身份证件（如身份证、护照或驾驶执照）照片；
                数字签名验证，以确保图片未经篡改。
            KYC3认证：高级安全验证
                除KYC2所要求的内容外，用户还需：
                上传一张手持身份证件的照片；
                通过数字签名验证确保图像真实有效。
            KYC1认证可实时完成；KYC2和KYC3需人工或系统审核，一般在几小时至几个工作日内完成，具体时长因平台而异。
            KYC认证流程的具体步骤
                1 收集客户信息
                2 验证身份
                3 背景调查
        ...
        CRS
            ...
            共同申报准则
            CRS 是由经济合作与发展组织（OECD）于2014年7月发布的全球性金融账户涉税信息自动交换标准。
            该标准旨在通过参与国家（地区）间的自动信息交换，打击跨境逃税行为，提升国际税务透明度
                根据CRS，各参与辖区的金融机构需识别非居民持有的金融账户，并按年收集、报送账户信息至本国税务机关，
            再由税务机关与其他国家（地区）的税务主管当局进行自动交换
            最终实现“跨境资产税务穿透”。
            打个比方：
                中国居民在新加坡某银行有存款，过去新加坡银行可能不会主动向中国税务机关披露这笔资金；
                但在CRS框架下，新加坡银行会收集该账户信息并提交给新加坡税务局，新加坡税务局再将信息同步给中国税务局，
                中国税务机关就能掌握这笔海外资产的情况，从而判断是否需要补税。
            截至2024年，全球已有100多个国家和地区加入CRS，包括中国、新加坡、英国、澳大利亚、加拿大等主要经济体，
            覆盖了95%以上的跨境金融账户。
            CRS的信息交换流程可概括为“识别—收集—报送—交换”四步：
                1. 识别非居民账户
                2. 收集账户信息
                3. 向本国税务机关报送
                4. 跨国自动交换信息
    各种指标
        量比
            现在的成交量，是最近平时成交量的几倍？
            【基本公式】
                量比 = （当日即时成交总量 / 累计开市时间） / （过去5日平均每分钟成交量）
            公式拆解：
                分子：当日即时成交总量 / 累计开市时间 = 当日的“平均每分钟成交量”。
                分母：过去5日平均每分钟成交量 = 最近5日的一个“常态成交水平”。
                比值：用“现在的每分钟平均成交量”除以“过去5日每分钟平均成交量”，得出的倍数就是量比。
            量比的数值含义与解读
                0.5 以下：显著缩量
                    市场状态：极度冷清。交易非常不活跃。
                    可能信号：
                        市场对该股暂时失去兴趣，关注度低。
                        可能处于阴跌、盘整或主力高度控盘状态，流动性差。
                        注意：在强势上涨后的高位缩量横盘，有时是危险的信号（无人接盘）。
                0.8 - 1.5 之间：正常水平
                    市场状态：成交量处于常态，没有异常资金活动。
                    可能信号：趋势大概率会延续之前的走势，没有特殊分析价值。
                1.5 - 2.5 之间：温和放量
                    市场状态：开始受到更多关注，资金流入/流出迹象初显。
                    可能信号：
                        如果股价同步上涨，这是健康上涨的标志，趋势有望延续（量价齐升）。
                        如果股价下跌，可能是调整开始，有一定抛压。
                2.5 - 5 之间：明显放量
                    市场状态：成交异常活跃，有大量资金在动作。
                    可能信号：
                        突破关键价位（如压力位、平台、整数关口）时出现，突破有效性大大增强。
                        注意结合价格位置：低位放量可能是主力建仓或启动信号；高位放量则可能是主力对倒或出货，风险加大。
                5 - 10 之间：剧烈放量
                    市场状态：成交极度活跃，通常有重大事件发生（如公布财报、重组消息、行业利好/利空）。
                    可能信号：
                        趋势可能进入加速阶段（暴涨或暴跌）。
                        反转警示：在长期上涨后高位出现，可能是“天量天价”，趋势即将反转。
                        需要极度警惕，并迅速结合消息面和其他技术指标判断。
                10 以上：极端放量
                    市场状态：成交达到罕见的活跃程度。
                    可能信号：
                        反向指标：这往往是趋势即将见顶或见底的强烈信号（物极必反）。
                        新股上市首日除外，其量比通常极高，不具参考性。
            量比的解读关键
                开盘量比：9:25集合竞价后的量比若超过3，表示该股当天动作可能较大，值得重点关注。
                股价位置：
                    低位高量比：低位高量比（>2.5）常视为资金介入信号，可能是主力建仓。
                    高位高量比：高位高量比往往是风险信号，暗示主力拉高派发筹码，成交量急剧放大但价格不涨。
                涨跌配合：上涨时量比适度增加代表健康的买入推动；下跌时量比放大则表明抛压沉重。
                缩量上涨：在放量突破后，量比变小（小于1）但股价持续创新高，通常表明主力控盘程度高
        换手率
            换手率 = 成交量 / 发行总股数（手）
        振幅
            （当日最高价 - 当日最低价） / 昨日收盘价
        ...
        成交额（Amount）
            指成交的实际资金金额（如元、亿元）。
        静态市盈率 P/E
            静态市盈率 = 当前总市值 / 上一年度的归属于母公司股东的净利润
            或
            静态市盈率 = 当前股价 / 上一年度的每股收益
        动态市盈率 P/E TTM
            滚动市盈率 = 当前总市值 / 最近四个季度归属于母公司股东的净利润之和
            或
            滚动市盈率 = 当前股价 / 最近四个季度的每股收益之和
        预测市盈率 Forward P/E
            预测市盈率 = 当前总市值 / 预测的未来年度归属于母公司股东的净利润
            或
            预测市盈率 = 当前股价 / 预测的未来年度每股收益
        市净率 P/B
            市净率 = 股价 / 每股净资产
        PEG
            市盈率相对盈利增长比率
        ...
        盈利增长率
            (当前每股收益 - 上期每股收益) / 上期每股收益
            或
            (当前利润 - 上期利润) / 上期利润
                这里的上期，可能是 上一个季度，上一年
            正增长（>0%）：表明公司业务在扩张，市场份额可能提升，成本控制有效或产品需求旺盛。
            高增长（例如 >15-20%）：通常被视为“成长型公司”，备受投资者青睐。
            负增长（<0%）：发出警示信号，
        股息率
            静态股息率（或历史股息率） = （上年每股现金股息 ÷ 当前股票市价） × 100%
            滚动股息率 = （过去12个月每股总股息 ÷ 当前股票市价） × 100%
        总股本
            流通股 + 限售股 (非流通股)
        流通股本
            可以自由买卖的股份
        ...
        分红
            ...
            转股
                把资本公积转股股票派发给股东
            派现 和 送股 必须要有利润，都是利润分配，要根据持股期限差异化征税
            转股有充足的资本公积就可以了，没有利润要求，一般也不需要纳税
            送股 和 转股 都不增加股东数量
        增发
            增加 资本公积 和 股东数量
        回购
        ...
        资本公积
        均线
            均线（Moving Average，简称 MA），又称移动平均线，
            是将某一段时间内的股票收盘价（或开盘价、最高价、最低价）相加后除以该时间段的天数，得到的一个平均值，
            然后将这些平均值依次连接起来形成的一条平滑曲线。
            核心本质：均线是对股价历史走势的"平滑化"处理，过滤掉短期价格波动的噪音，反映出股价在一段时间内的平均成本和趋势方向。
            短期
                5日 10日 20日 30日
            长期
                60日 120日 250日
            类型
                ...
                加权移动平均线（WMA）
            形态
                金叉	短期均线上穿长期均线	买入信号，趋势可能转多
                死叉	短期均线下穿长期均线	卖出信号，趋势可能转空
        k线
            分时 日线 周线 月线
            组成
                颜色
            ...
            形态
                单个 两个 三个 持续形态
        支撑位（Support Level）
            指股价在下跌过程中，遇到买方力量增强、卖方力量减弱，从而阻止股价继续下跌的价位或区域。
            可以理解为股价下跌时的“地板”。当股价触及该位置时，通常会引发反弹或横盘整理。
        压力位（Resistance Level）
            指股价在上涨过程中，遇到卖方力量增强、买方力量减弱，从而阻止股价继续上涨的价位或区域。可以理解为股价上涨时的“天花板”。
            当股价触及该位置时，通常会遭遇回落或横盘整理。
        如何识别 支撑位 和 压力位
            前期高点与低点
            均线
            成交量密集区（筹码峰）
    ...
    股票类型
    估值（Stock valuation）
    交易理论和策略（Trading theories and strategies）
    股票交易时的费用
        佣金   证券公司
        印花税 国家税务机关
        过户费 中国证券登记结算公司
        经手费 交易所
        证管费 中国证监会
        过户费 经手费 证管费 通常十分的少，通常会合并在 其它费用 中
            在股票交易软件中通常只显示 佣金 印花税 其它费用
    什么是滑点（Slippage）？
        核心定义
            滑点 是指订单的预期执行价格与实际执行价格之间的差异
        正面滑点：实际成交价格比预期更好
            买入时，实际价格低于预期价格
            卖出时，实际价格高于预期价格
        负面滑点：实际成交价格比预期更差
    ...
    股票交易和分析软件
        同花顺 大智慧 指南针 东方财富 信达通 钱龙 腾讯自选股 雪球 集思录
    ...
    同花顺都有哪些产品和服务
        ...
        增值服务
            神奇电波 仙人指路 财富密码 走势预测
            尾盘致胜 涨停密探 AI波段 筹码先锋 超能形态
            神奇九转 龙虎点睛 决策助手 主力密码 龙头强度 形态掘金 波段雷达
    ...
    github 相关的仓库
        ...
        LLM驱动的 A 股智能分析器 https://github.com/ZhuLinsen/daily_stock_analysis
        ...
        一个自托管的量化交易平台 https://github.com/brokermr810/QuantDinger
            内置的基于 LLM 的多智能体研究系统
            数据来自  AkShare, 东方财富
        多智能体金融应用平台 https://github.com/ValueCell-ai/valuecell
财务
    会计(Accounting) 出纳(cashier) 审计(audit) 税务(taxation，Tax Accounting)
    cashier 也有 收银员的意思
    ...
    会计要素 Accounting Elements
        中国 CAS
            资产（Assets）、负债（Liabilities）、所有者权益（Owner’s Equity）、收入（Revenue）、费用（Expenses）、利润（Profit） 六要素
        台湾
            资产（Assets）、负债（Liabilities）、业主权益（Owner’s Equity）、损益计算（Profit or Loss） 四要素
        国际会计准则 IFRS
            资产（Assets）、负债（Liabilities）、产权（Equity）、收益（Income）、费用（Expenses） 五要素
        美国财务会计准则 GAAP
            资产、负债、权益、业主投资、派给业主款、总收益、营业收入、费用、利得、损失 十要素
    流水 成本 营业额 营业成本 营业收入 营业利润 税前利润 净利润 利润 毛利润 毛利率
        口语化的表述 会计中的标准术语
            流水是口语，约等于 营业收入
            营业额是口语，约等于 营业收入
        成本 就是 支出 就是 费用 ？
        ...
        毛利润
            ...
            profit n. 利润
            未扣除间接费用或经营费用的初步利润。
            还没减掉管理费用、销售费用、财务费用、税费等间接或经营性支出，因此是“粗略”的、未精算的利润数值。
            毛”字还有“毛坯”、“毛皮”等用法，带有未加工、未精整之意，和这里“未扣除各项费用”的含义一致。
            毛利润 = 营业收入 − 直接成本
            营业利润 = 毛利润 − 营业费用
        毛利率 = （毛利润 / 营业收入） × 100%
        营业成本（Cost of Revenue）：直接与收入配比的成本，如商品成本、劳务成本
        营业费用（Operating Expenses）：期间费用，如销售费用、管理费用，与营业成本是不同科目
...
电子支付，区块链，数字货币
    ...
    除了比特币之外的主要的数字货币
        ...
        USD Coin（USDC） 另一种与美元挂钩的数字稳定币，类似于 USDT
    比特币的衍生币
    以太坊(Ethereum)
...
FIRE和财富自由和躺平
    ...
    财富自由
        ...
        被动收入 大于 日常支出+大额支出（买房买车结婚这类）+应急储备
    躺平
        定义1：财富自由
    ...
    FIRE
        定义
            ...
            FIRE运动在Reddit有很多讨论
        分类
            Morbidly Obese FIRE 病态 肥胖 FIRE
                比 ObeseFIRE 更富裕的 FIRE
                资产规模 5000万美元起步，通常在 1亿 - 10亿美元以上
            ObeseFIRE 肥胖FIRE
                比 FatFIRE 更富裕的 FIRE
                资产规模 UHNW
            FatFIRE 肥FIRE
                ...
                资产规模 VHNW
            ChubbyFIRE 微胖FIRE
                指代“比普通FIRE富裕，但比Fat FIRE稍微收敛一点”的中间状态
                资产规模 HNW
            RegularFIRE 传统FIRE
...
excel 相关经验
    ...
    公式
        ...
        roundup   向上取整
        rounddown 向下取整
        CEILING.MATH 四舍五入到制定倍数
            例如 k4=1236 CEILING.MATH(k4, 10)=1240
        exp 计算 自然常数 exp(1) 就是 e 的值
        统计
    ...
    格式
        ...
        wps 的格式应该也可以打开
    语法
        2003 2007 2010 2014 2016 2019 2021 2024 365
        版本太古老的就不理了
...
整理各个 api 的资料
    ...
    mistral
        ...
        只是偶然有免费模型，多数情况下依然收费
    openrouter 也有不少免费的模型，但免费模型不能调用 tools
        https://openrouter.ai/models?max_price=0&fmt=table
        ...
        不充钱的情况下，每天只有50次；充值 10 credits 后可解锁每天 1000 次，充值可以用 支付宝 或 微信支付
    硅基流动
        墙内版有免费模型，但需要实名认证才能用
        国际版没有免费模型
        https://siliconflow.cn/pricing
    腾讯云 这个是收费的
    智谱 有一个免费的模型可以用，当前是 glm-4.7-flash
    这几个也有免费的api提供
        nvidia
        Google AI Studio
        https://modelscope.cn/
...
Instant
    ...
    in s 等
    in 私 等
例句助记： instant noodles（方便面）—— /ˈɪnstənt ˈnuːdlz/
...
experiment
实验 n. v.
英式发音：/ɪkˈsper.ɪ.mənt/（或 /ekˈsper.ɪ.mənt/）
美式发音：/ɪkˈsper.ə.mənt/
分解读法（按音节）：
    ex（/ɪks/ 或 /eks/） + per（/spɜː/ 或 /spɜr/，重音在此） + i（/ɪ/ 或 /ə/） + ment（/mənt/）
...
Claude
    国际音标：/klɔːd/
    汉语近似发音：克洛德
    克劳德·艾尔伍德·香农（Claude Elwood Shannon） 就是信息论的那个香农
Anthropic
    国际音标：/ænˈθrɒpɪk/
    汉语近似发音：
        安·斯罗皮克（重音在“斯罗”）
        ...
        an 斯罗 匹
    希腊语 anthrōpikos（人类的）
    人择原理	公司名称，追求以人为中心的 AI
    公司名称取自 Anthropic Principle（人择原理）——该原理认为宇宙的基本参数恰好允许人类存在，并非巧合。
    Anthropic 以此命名，寓意其致力于构建“对人类本质有深刻理解、以人类价值观为导向”的 AI 系统。
Haiku
    国际音标：/ˈhaɪkuː/
    汉语近似发音：海·库
    日语 俳句（极短诗）
Sonnet
    ...
    汉语近似发音：索·尼特
    十四行诗（中长诗）
    最初源于意大利语的 sonetto，意为“小曲”或“小歌”。
    这个词在13世纪时随这种诗体一同诞生，后来经过演变进入法语（sonnet），并最终被英语吸收作为诗歌术语
Opus
    国际音标：/ˈəʊpəs/
    汉语近似发音：欧·帕斯
    拉丁语 艺术作品
...
mistral 这些是通用模型
    large
    medium
    small   虽然叫 small ，但参数规模也有 120b
Devstral 这个是 code 专用的模型 类似于 qwen coder
Ministral 这个是 小尺寸模型 类似于 phi gemma


Mistral
    国际音标：/ˈmɪstrəl/ 或 /mɪsˈtrɑːl/
    汉语近似发音：米斯特拉尔（重音在“米”或“拉尔”）
    mi stral
Devstral
    这个词不常见，可能是自造词或品牌名。根据拼写推测：
    国际音标：/ˈdɛvstrəl/
    汉语近似发音：德夫斯特拉尔
    dev stral
Ministral
    同样不常见，可能是自造词或品牌名。根据拼写推测：
    国际音标：/ˈmɪnɪstrəl/
    汉语近似发音：米尼斯特拉尔
    mini stral
...
美式发音 [mjuz]
谐音读作：“谬斯”
名词：灵感；希腊神话中的缪斯女神（主管文艺与科学的九位女神之一）。
动词：沉思、冥想
...
程序员 常见单词的发音
https://pronounce.renlab.ai/zh
https://github.com/anzy-renlab-ai/pronounce


墙内 gpt Claude gemini 都无法使用，
在 openrouter 请求这几款模型会提示 403
...
导出为 Markdown JSON Text HTML 格式

从文档里看，还可以在指定 llm 模型来完成对 pdf 或 图片的 orc 识别
...
python -m docling --from pdf --to md test.pdf

https://github.com/microsoft/markitdown
MarkItDown是一个轻量级的Python工具，用于将各种文件转换为Markdown。
...
python -m markitdown test.pdf -o test.md
...
2014 年 9 月， 论文《 Neural Machine Translation by Jointly Learning to Align and Translate》首次提出 注意力机制（Attention Mechanism）
2015 年，OpenAI 成立。
2016 年，DeepMind 开发的 AlphaGo 4:1 击败韩国围棋冠军李世石。 AlphaGo使用了强化学习
2017 年，DeepMind 团队发表《Attention Is All You Need》论文，提出 Transformer 的新型神经网络模型。
2018 年，GPT-1 发布
2019 年，GPT-2 发布
2020 年，GPT-3 发布
2022 年 1 月 27 日 InstructGPT text-davinci-001
2022 年 3 月 15 日 GPT-3.5 text-davinci-002 上线API
2022 年 11 月 28 日 GPT-3.5 text-davinci-003
2022 年 11 月 30 日，ChatGPT
2023 年 3 月 1 日，gpt-3.5-turbo
2023 年 3 月 14 日，GPT-4 
2023 年 6 月 13 日，gpt-3.5-turbo-16k
2023 年 9 月 18 日，gpt-3.5-turbo-instruct
2024 年 5 月，GPT-4o 发布
2024 年 9 月，GPT-o1 发布
2025 年 2 月，GPT-4.5 发布
2025 年 4 月，GPT-4.1 发布
2025 年 4 月 16 日，o3 发布
2025 年 5 月，GTP-5 和 GPT-OSS
2025 年 11 月 12 日，GPT-5.1 发布
2025 年 12 月 11 日，GPT-5.2 发布
2026 年 2 月 5 日，GPT-5.3 Codex 发布
2026 年 3 月 4 日，GPT-5.3 Instant/Chat 发布
2026 年 3 月 5 日，GPT-5.4 发布
2026 年 4 月 24 日，GPT-5.5 发布
2026 年 7 月 10 日，GPT-5.6 发布

4.1 确实比 4.5 晚发布
GPT-5.3 Instant/Chat 和 GPT-5.4 发布的时间就隔了一天，大约相隔48小时
GPT-5.3 作为旗舰模型 仅仅 保持了一个月

为什么 gpt 没有 o2 模型
公开报道（TechCrunch、Gizmodo 等）说 OpenAI 为避免与英国电信品牌“O2”发生潜在商标/命名冲突，选择跳过 “o2”，直接命名为 o3

2023年7月：DeepSeek成立，总部位于杭州
2023-11-29 DeepSeek LLM
DeepSeek-V2-0517
...
2025-12-01 DeepSeek-V3.2 DeepSeek-V3.2-Speciale
    V3.2-Speciale 是 DeepSeek-V3.2 的长思考增强版
    Speciale 仅以临时 API 服务形式开放，以供社区评测与研究
2026-04-24 DeepSeek-V4-Pro DeepSeek-V4-Flash
2026-07-31 DeepSeek-V4-Flash-0731
2026-08-12 DeepSeek-V4-Pro-0813
2026-08-13 DeepSeek Harness
2026-08-21 DeepSeek-V4-Flash-Vision-Exp

还有 deepseek-Coder deepseek-Math deepseek-OCR deepseek-VL


基础模型（英语：foundation model或base model）
通常简称 FM
...
基础 训练 提示词工程 上下文工程 Embedding/rag/知识库 微调 tools mcp ChatBot/Workflow/Agent/subagent/multi-agent skills 评估
本地部署llm
使用 氛围编程（Vibe Coding） 加速开发

思考模式 和 普通模式有什么区别？
    具体表现是 模型先输出一段 思维链内容，再输出答案
什么样的模型会有思考模式？
...
jan https://www.jan.ai/ 类似于 lm studio ，模型是从 Hugging Face 下载的
...
tngtech
一个 德国的企业
发布了基于DeepSeek开发的R1T2



https://github.com/allenai
OLMo，它的英文全称就叫Open Language Model。
相比之前开源的大模型，OLMo的独特之处是完全开源，
除了训练的模型，OLMo还开源了训练数据，训练代码以及模型评估代码。
换句话说，只要你有GPU卡，再基于OLMo开源的数据和代码，就可以自己快速从零训练一个自己的LLM。
...
任务设计书/开题报告 同样也可以用 大模型生成
使用 氛围编程 (Vibe Coding) 完成代码部分
使用 本文文本 完成论文部分
最后再把 md 转换为 docx （这一步同样可以用大模型实现）
整合成一个 工作流
    输入 标题 技术栈 论文要求
    输出 任务设计书/开题报告 代码 论文 ppt 答辩演讲稿

氛围编程 (Vibe Coding) 中如何写 space ？
...
最初 vLLM 是在加州大学伯克利分校的天空计算实验室 (Sky Computing Lab) 开发的，
如今已发展成为一个由学术界和工业界共同贡献的社区驱动项目。

其实我没有看懂 vLLM 是什么来的。。。


十分好的网站，有大量 ai 相关文章，教程，还有一个显存计算器
https://apxml.com/zh


十分好的博客，有大量和ai相关的内容
https://jimmysong.io/zh/blog/
...
AI Agent 框架
    ...
    https://github.com/langflow-ai 重点关注这个
        这个居然不支持自定义的 llm ，很坑。。。我感到很失望
        通过修改 openrouter 那个组件，然后再保存为自定义的组件，面前实现自定义的 llm
        https://docs.langflow.org/bundles-openrouter
        https://docs.langflow.org/components-models
    https://github.com/langchain-ai/langchain
    https://github.com/chatchat-space/Langchain-Chatchat 这个可能不是最好的，但有中文


上下文工程 和 提示词工程 的区别在哪里？
...
开源编辑器 Zed 宣布推出全新的 Agent Client Protocol (ACP)，
这一协议让开发者可以在编辑器中自由接入任意第三方智能体（Agent），无需依赖官方内置工具。
其理念类似于 语言服务器协议（LSP），通过解耦编辑器与智能体的交互方式，提供更灵活的扩展能力。
旨在为AI智能体与集成开发环境(IDE)的整合提供标准化方案，防止开发者被锁定在VS Code中。
首个落地案例是与 Google Gemini CLI 的合作。
...
外国人习惯用art（艺术）这个词来比喻最先进、最前沿的东西。



低代码/无代码/可视化编程/工作流/rpa
...
工作流
    jbpm
        ...
        Business Process Modeling Notation BPMN 业务流程建模与标注
    camunda
        Camunda有Camunda7和Camunda8两个版本
        Camunda7基于activiti5发展来来
            除了开源版本同时也提供了商业版，不过对于一般企业应用，开源版本也足够了
        Camunda8定位于云架构SaaS模式，Camunda7和Camunda8在技术架构方面有本质区别
    activiti
        Activiti的源头是由JBPM4流程引擎发展而来
        activiti包括activiti5、activiti6、activiti7三个版本
        activiti5和activiti6的代码在github上已经4年没有更新了，新开发项目不建议选择activiti5以及ativiti6
        activiti7即Activiti Cloud：定位云产品，完全面向云原生架构设计开发
    flowable
        flowable分开源版和商业版
...
可视化编程
    ...
    属性
        表单的属性，组件的属性
    事件
        事件就是对一个组件的操作，如用鼠标点击一个按钮，在这里，点击鼠标就称为一个事件(Click事件)。
    方法
        方法就是某个事件发生后要执行的具体操作
...
React DnD 是一套 React 实用工具，可帮助您构建复杂的拖放界面，同时保持组件的解耦。
适合实现低代码平台
https://maqib.cn/blog/react-dnd-low-code
...
它的核心思路针对业务需求进行数据建模：
根据业务需求提炼出类，然后通过 ORM 把类映射为表结构，
并根据读写性能要求使用范式优化表与表之间的关联关系。
数据驱动是从技术的维度解决业务问题，得出的数据模型是对业务需求的直接翻译，
并没有蕴含稳定的领域知识/规则。
一旦需求发生变化，数据模型就得发生变化，对应的库表的设计也需要进行调整。
这种设计思维导致变化从需求穿透到了数据层，中间并没有稳定的，
不易变的层级进行阻隔，最终导致系统响应变化的能力很差。
...
低代码/无代码 本质上是为了使编程工具更接近人的原本的思维

低代码/无代码 应该专注于 mvp 的实现？
又或者是 成熟系统的拓展？
感觉 magento2 里那种 condition 就已经很接近无代码了
旧时代的各种 工作流引擎 ？
...
写一份 php 和 js 都能公用的模板引擎？
写一个类似于 fpm 的 js 框架？
两个方向？
    让 js 写后端
    令后端的语言也能运行在前端，能操作 dom 的那种
前端 后端 的运行环境不一样，即使是同一种语言，代码也未必能复用
...
创业的方向
    软件的
        ...
        正则化领域知识 （例如 菜谱）
        互联网算命 ai 算命？
            紫微斗数排盘信息 https://github.com/SylarLong/iztro
            https://www.v2ex.com/t/1118056 https://github.com/x-haose/py-iztro
    硬件的
        写字机器
            账簿 ， 作业 ，检讨书
            怎样识别可以书写的位置？
        ...
        nas
        硬盘盒
        htpc（Home Theater Personal Computer），即家庭影院PC
        机箱
        信息箱
        准系统
参考同类项目？
...
小众的（小众的才不会被留意到）
不一定要在墙内
不能服务于程序员（因为程序员看到收费肯定是想自己写一个而不是付费）
...
https://github.com/liu-ziting/ThinkFlowAI
ThinkFlow AI 是一个轻量、偏本地化（Local-first）的“从想法到结构化图谱”的工作台：
输入一个核心想法，应用会把它扩展为可浏览的知识图谱（模块/子模块）。
每个节点支持继续追问扩展、深挖详情，以及可选的配图生成。



llm 从写代码的角度看就是一个 api
而且是一个输出不怎么稳定的 api
...
nanobot是一个超轻量级的个人人工智能助理，灵感来自Clawdbot
...
https://openclaw.ai/
没找到具体数据，不过我猜测 2h4g应该够用的
至少有放行 18789 18790

生成32位随机字符串
awk 'BEGIN{srand();s="0123456789abcdefghijklmnopqrstuvwxyz"; for(i=0;i<32;i++) printf "%s", substr(s, int(rand()*36)+1, 1); print ""}'
tr -dc 'a-z0-9' < /dev/urandom | head -c 32; echo

需要设置这两个参数，才可以不通过 localhost 访问 dashboard
openclaw config set "gateway.auth.token" 'trl7fpvdblt7lwmqfnvzu2y5xh29no96'
openclaw config set "gateway.controlUi" '{"allowInsecureAuth" : true}'

修改完配置后要记得重启 gateway
openclaw gateway restart

使用 socat 把 18789端口 临时暴露在公网
nohup \
    socat tcp-listen:8002,reuseaddr,fork \
    TCP:127.0.0.1:18789 \
&
...
配置 llm
  "models": {
    ...
    "providers": {
      "qcloudlkeap": {
        ...
        "apiKey": "sk-w9oenTtxfpPH2vMLaSrsy37Ryt1uuupEiHEjNTgFVHsTJDzj",
  ...
  "agents": {
    "defaults": {
      "model": {
        "primary": "qcloudlkeap/deepseek-r1-0528"
      },
      "models": {
...
可以通过这几个命令查看 openclaw 的状态
openclaw doctor
openclaw status
openclaw status --all


如果在 dashboard 里没有效果，dashboard其实还很简陋，很多配置都只能通过json修改
可以通过 tui 试试，tui会输出更多信息
openclaw tui


安装 plugin 之前要先看一下 plugin 是否已存在
openclaw plugins list
openclaw plugins install @openclaw/nextcloud-talk


删除对应的文件夹就是卸载 plugin 了
从这个命令中找到对应的文件夹
openclaw plugins list


openrouter 的免费模型不支持 tool
所以，之前一直失败
openclaw 又没有这方面的兼容，默认所有模型都支持 tool
所以，失败了也没有明确的报错
...
Nextcloud talk
    服务端
        ...
        可以通过 Matterbridge 连接外部服务（如 Matrix、Discord）
        最小配置 2h2g
            服务端勉强部署起来，但速度十分的慢，需要很多个容器
        需要
            80 8080 8443 3478tcp 3478udp
    客户端 https://github.com/nextcloud/talk-android
    f-droid ✅
    google play ✅
    web ✅
Matrix
    ...
    最后选择了
        服务端 Dendrite
        客户端 fluffychat
            直接从 google play 下载就好了
            没有消息推送
            似乎不能很好地显示 markdown 格式
        matrix 的客户端如果需要消息推送似乎还需要一个 UnifiedPush
            UnifiedPush：一种去中心化的开源推送通知协议 https://f-droid.org/zh_Hans/2022/12/18/unifiedpush.html
            https://unifiedpush.org
            UnifiedPush 同样也区分 客户端 和 服务端
                ntfy 项目包含 ios 安卓 windows 等多个客户端和服务端
                客户端 可以通过 f-droid 或 google play 下载
                服务端 https://docs.ntfy.sh
tlon 似乎也可以
    https://github.com/tloncorp/tlon-apps
...
# 这一步要注意修改域名
sed -i 's/YourDomainHere/example.domain/g' ./config/dendrite.yaml

# 这一步要注意修改secret
awk 'BEGIN{srand();s="0123456789abcdefghijklmnopqrstuvwxyz"; for(i=0;i<32;i++) printf "%s", substr(s, int(rand()*36)+1, 1); print ""}'
sed -i 's/registration_shared_secret: ""/registration_shared_secret: "m4pm1ulmu3dkk9sq"/g' ./config/dendrite.yaml


curl -O https://raw.githubusercontent.com/element-hq/dendrite/refs/heads/main/build/docker/docker-compose.yml
...
docker ps -a
新建用户， -admin 是管理员，但管理员似乎无法参与聊天？其实没有管理员也可以的
docker exec -it 容器id /bin/sh
cd /etc/dendrite
/usr/bin/create-account --config /etc/dendrite/dendrite.yaml --username alice -admin


docker exec -it 0d7c1c45d50f /usr/bin/create-account --config /etc/dendrite/dendrite.yaml --username alice -admin
docker exec -it 0d7c1c45d50f /usr/bin/create-account --config /etc/dendrite/dendrite.yaml --username user1
docker exec -it 0d7c1c45d50f /usr/bin/create-account --config /etc/dendrite/dendrite.yaml --username chatbot
命令运行完后要记得记下 token

如果有https，则可以使用这个网址验证服务端
app.element.io


在 openclaw 中启用 matrix
...
当前的版本，修改完后依然启动失败
是因为缺少了 这个模块 @vector-im/matrix-bot-sdk
重新安装一次
...
如果启动失败，可以尝试通过这个命令寻找原因
openclaw doctor


pairing（默认）：收到未知用户消息时发送配对码，需管理员批准后才能交互。
openclaw pairing approve matrix RWV653WF
...
无法启动浏览器
    ...
    /root/.cache/ms-playwright/chromium_headless_shell-1208/chrome-headless-shell-linux64/chrome-headless-shell \
        ...
        --disable-gpu \
        --user-data-dir=/tmp/openclaw-test
    虽然能成功启动浏览器，却没有返回信息
    如果没有明确声明，基本不会自己调用 浏览器，难道是大模型太蠢了？
    这里的 python3 playwright 都是在宿主机全局安装的
...
用浏览器打开 百度首页，并返回html代码
用浏览器打开 百度首页，搜索 openclaw ，抓取前十条搜索结果


dashborad 作用不是很大，配置文件基本靠直接修改


还是需要一个域名，不论是 openclaw 还是 matrix
再研究一下 windows 环境怎么安装
    wsl2
    msys2
...
先创建硬盘
再启动系统
    qemu-system-x86_64 debian13.img \
        -usbdevice tablet \
        -m 4096 -smp 12 \
        -netdev user,id=mynic0,hostfwd=tcp::2289-:3390 \
        -device virtio-net-pci,netdev=mynic0,mac=52:54:98:76:54:32 \
        -usbdevice tablet -accel whpx,kernel-irqchip=off \
        -cdrom "C:\debian-13.3.0-amd64-DVD-1.iso"
系统启动后设置 source
    到清华的镜像站换源
...
桌面环境还要解决
    ...
    中文输入法
    和宿主机互通的剪切板
        映射一个端口出来，通过远程桌面连接，这样就可以和宿主机共享剪切板了
    混乱的权限管理
给 python 加上一个软连接
    ln -s /usr/bin/python3 /usr/bin/python;
换源之后安装 pip
    apt install -y pip
从 nodejs 的官网，根据提示用 nvm 安装 node 
安装 Git
    apt install -y git
根据 openclaw 的提示安装 openclaw
    npm i -g openclaw
    openclaw onboard 第一次配置
    openclaw dashboard 打开网页后台

qemu 的命令行参数 稍微改动一点，经验又失效了。。。


llm 升级一下，一些工程上的优化和技巧又会失效了。。。


git clone https://github.com/HKUDS/nanobot.git
git clone --single-branch --no-tags https://github.com/HKUDS/nanobot.git --depth=1
cd nanobot
...
同样也是无法使用 openrouter 的免费模型，因为 nanobot 也是需要 tool

nanobot 支持邮件
    但要怎么区分会话？
nanobot 默认情况下无法调用浏览器
nanobot 默认情况下没有webui
nanobot 可以实现多agent吗？
nanobot 如何调用subagent？
nanobot 大致的目录结构
https://github.com/HKUDS/nanobot/discussions/460
...
OpenAkita ，一个类似OpenClaw（ClawdBot）的开源项目。

NanoClaw
https://github.com/qwibitai/nanoclaw
用 TypeScript 实现的 nanoclaw

moonclaw
https://github.com/vectie/moonclaw
用 moonbit 实现的 nanoclaw
...
一个开源的生产级平台，用于构建 AI 驱动的即时通信机器人
https://github.com/langbot-app/LangBot

CowAgent 是基于大模型的超级AI助理
...
https://github.com/robertgdev/LaraClaw
PHP/Laravel 写的 claw
只有几个星，其中一个还是我给的。。。
...
providers model agents channels plugins skills tools(fc/mcp) memory
webui 任务管理 


openclaw 是用 js/ts 实现
nanobot 是用 python 实现
picoclaw 是用 go 实现，而且 picoclaw 是用 opencode 自举的，大概用了 20m token

nanobot 这类工具主要是依靠 exec shell tool 这类 function call 来执行本地命令
https://github.com/HKUDS/nanobot/blob/main/nanobot/agent/tools/shell.py


openclaw 和 Manus 的区别是什么？
    openclaw 主要使用 im交互，Manus 主要使用 命令行或网页交互
    openclaw 主要使用 skills ， Manus 主要使用 function call 和 mcp
        用 skills 扩展功能比 function call 和 mcp 容易
    openclaw 主要运行在本地，Manus 主要运行在云端
    openclaw 有定时任务
为什么 openclaw 能这么出圈，同样开源的 openManus 却没那么火？
    其实 Manus 当初也很出圈，只是 Manus 热度过去了


claw会不会出现一个标准，类似 mcp 或 skill 这样的标准？
...
不同类型的显卡
    英伟达 amd 英特尔
一个新的电脑十分昂贵，还要考虑电费
同样的价格换算成调用api，大概可以用多久？
如果一天24小时都在运行，很快就能省出一个 主机了，但是有什么需求要一天都在运行？
而且本地部署的推理水平肯定不如云端的



有哪些可以替代 cowork 的开源项目？

只要把问题复制多次，就能提升准确率？


Function Calling 的工作流程？
模型选工具→生成参数→代码执行→结果返回
...
微调 Fine-tuning
    全量微调（Full Fine-Tuning，FFT）：
        更新模型的所有参数。效果好，但需要的 GPU 显存和全量训练一样大。
        7B 模型全量微调需要约 60GB+ 显存。
    参数高效微调（Parameter-Efficient Fine-Tuning，PEFT）：
        只更新一小部分参数，效果接近全量微调但资源需求大幅降低。
大多数语境下的微调是指 参数高效微调（PEFT）
低秩矩阵分解 LoRA（Low-Rank Adaptation） 是目前最流行的 PEFT 方法。

QLoRA（Quantized LoRA）：消费级显卡也能微调
QLoRA = 量化 + LoRA
...
Qwen 3.5-122B-A10B
这个模型名称中，后缀 A10B 指的是该模型采用了混合专家（MoE）架构，
其中激活的参数数量为100亿（10 Billion）
总参数 1220亿（122 Billion）
激活参数 100亿（10 Billion）
因此，Qwen3.5-122B-A10B 的含义是：
这是一个总参数量高达 1220亿（122B） 的模型，
但由于采用了高效的MoE架构，它在处理每个请求时，
仅动态激活其中的 100亿（10B） 个参数进行计算 。
这种设计的优势在于，它既能拥有大容量模型的强大能力，
又能显著提升推理效率、降低算力成本，甚至可以在配置较高的消费级显卡上运行


语言模型的规模越大，量化后的损失越小
    70b 及以上的模型 即使是 int4 量化，损失也只有1%左右
    中间规模的模型
        int8 3%左右的损失
        int4 5%左右的损失
    8b 及以下的模型，量化后输出质量会大幅下降（大于10%）
通常情况下，量化后的模型速度会更快
量化不一定都是为了节约显存，也可能是为了更快的速度
...
参数
    如果是 稠密模型，参数规模 * 类型
    如果是 moe模型， 参数规模 * 类型 + 激活参数 * 类型
kv
    2 * 批次大小 * 序列长度 * 类型 * 隐藏维度 * 层数
激活值
    如果是 稠密模型，那么就约等于 参数的10%
    如果是 moe模型，一般会明确声明的
还要预留一点冗余
...
隐藏维度 好像没有什么规律 最好还是认真看一下文档 每个模型的值似乎都有一点差异
        如果只是简单估算 那么就取值 8192
层数     好像没有什么规律 最好还是认真看一下文档 每个模型的值似乎都有一点差异
        如果只是简单估算 那么就取值 80


稠密模型（Dense Model） 和 混合专家模型（MoE, Mixture of Experts）
是当前构建大型语言模型（LLM）的两种主流架构。
它们最核心的区别在于参数的使用方式：
稠密模型处理每次请求都会激活所有参数，
而MoE模型只会动态激活其中一小部分参数。
...
中间道路：稠密训练，稀疏推理
DS-MoE 范式：
训练时所有专家激活（稠密式梯度流向每个专家），
推理时切换到稀疏 Top-K

小模型 使用 Dense 更好

如果只是想在自己的机器上跑一个模型玩玩，Dense 是更务实的选择。 
MoE 的优势在大规模部署时才真正体现出来。

MoE 的量化，规模参数 和 激活参数 可以使用不同的精度
例如 规模参数使用int4量化，激活参数使用int8量化

moe 在部署时，可以使用 混合部署 或 专家缓存 这类技术
把 规模参数 加载到内存，激活参数加载到显存
所以，理论上 只需要300g内存+16g显存就能运行 deepseek v4 flash

一个值得关注的趋势：
MoE 和 Dense 不是非此即彼。
很多最新的模型在同一架构里混合使用——
attention 层 Dense，FFN 层 MoE；
浅层 Dense，深层 MoE。
DeepSeek-V4 就是这种混合思路的典型。
...
应该是 市面上 第一个 做 深度研究 的 公司
免费版每月只有一次机会
免费版只能聊天
https://www.perplexity.ai

Sonar 是 perplexity 自研的大模型



leap
liquid edge ai platform 
liquid 边缘 ai 平台
...
Unsloth 让你在自己的本地硬件上运行和训练 AI 模型。
https://unsloth.ai
...
前沿部署工程师
核心职责
    驻扎在客户现场，深入解构企业的具体业务流程（如财务、供应链、客服）
    将大模型或 AI Agent 与客户现有的复杂系统（ERP、数据库）对接，编写轻量级的“粘合剂”代码，将其转化为生产力
    总结现场的具体需求，提炼成通用功能带回总部，让通用产品越来越强大
能力要求
    扎实的工程能力
    卓越的沟通
    敏锐的商业嗅觉
过去 AI 公司卖 API（Token），但企业客户发现通用模型“无法直接用”。
FDE 的出现填补了“产品能做什么”和“客户需要什么”之间的巨大鸿沟，
是 AI 公司 To B 商业化变现的最关键角色
...
后训练一般会经过三次微调
    第一次 使模型从 base 向 对话模型 或 指令模型 转换
    第二次 是人类价值观对齐（对齐偏好，价值观对齐，安全与道德对齐）
    第三次 是加上 思维链（CoT）

有思维链的就可以算是推理模型，
没有思维链的一般叫做 标准模型 或 传统模型 或 非推理模型
思维链在推理过程中产生的token也计费的，即使这些token不会直接显示给用户，
而且很多时候推理产生的token比输出产生的token还多

非推理模型
    直接输出最终答案 (Input → Output)
推理模型
    先生成思维链，再输出答案 (Input → CoT → Output)
    推理模型在给出答案前，会进行显式的、多步的逻辑推演和自我反思。


后训练阶段一：SFT（监督微调）
预训练模型会续写文本，但不会遵循指令。
    你问它"北京有什么好吃的？"，它可能会继续写"北京有什么好玩的？上海有什么好吃的？"——因为它只是在做 Next Token Prediction，
    不知道你在问问题。
...
base 模型就是没有经过 后训练的模型

eval
evaluation
评估

agent的评估 和 llm的评估 是不一样的
...
AI4AI
AI for AI
“AI For AI”是指利用人工智能来构建和优化其他人工智能系统，这能提高效率，
但同时也引发了伦理和数据相关的担忧。 
...
旧时代的虚拟助理（virtual assistant）
    ...
    小爱 Xiaoai 
    小度 XIAODU
    小艺 Celia（中国大陆以外地区使用），或 XiaoYi
    小冰 Xiaoice
    天猫精灵 TmallGenie


语音控制（Voice Control） -> 虚拟助理（virtual assistant） -> llm
语音控制
    是一种输入信号
    通常是通过关键词匹配，把“声音”转换成具体的操作，本质上和“用手指触摸屏幕”或“敲击键盘”没有区别。
    通常不需要联网
虚拟助理
    语音控制的基础上加上语义理解，意图识别
    可以使用自然语言交互
