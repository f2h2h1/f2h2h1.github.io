...
规范模式（canonical mode） 就是 行模式
原始模式（raw mode） 就是 字符模式
块模式（Block mode） = 原始模式 + 流控关闭
    全屏/图形终端应用
    ssh telnet 这类程序会用到
    TUI 这类程序应该也算是吧

canonical mode 也被称为 cooked mode
因为 cooked（熟的） 是相对 raw（生的）



终端驱动与行输入处理由内核的 line discipline 控制
Shell 或其他程序可以通过 POSIX termios API（或旧的 ioctl/termio 接口） 配置终端的行为
本质上是修改
    tty驱动的配置 （虚拟终端）
    pty的配置 （伪终端）
    这两个都是运行在服务器本地的
远程终端是怎么和 telnet 或 ssh 交互的？
...
git for windows 中 的 bash
- /usr/bin/mintty.exe --> 单独的 终端
- /usr/bin/bash.exe --> 单独的 shell
- /bin/bash.exe --> /usr/bin/bash.exe + 加上一些必要的环境变量
    - 调整 PATH 让 Git 的工具（/mingw64/bin、/usr/bin）排在 Windows 系统目录前面
    - 用于集成到 VS Code、Windows Terminal 等工具
- git-bash.exe --> /usr/bin/mintty.exe + /bin/bash.exe
    - 具体位置在 git for windows 的安装根目录下
    - 一般用于 开始菜单 任务栏 桌面图标 右键菜单
- 运行在 vscode 终端里的是 `/bin/bash.exe -i -l`
    - 没有 -i -l 这两个参数会缺失一些命令
- 直接运行 bash.exe 也会出现一个命令行界面
    - 对于 windows 而言直接运行 bash.exe ，是一个 控制台进程，所以也会有一个 命令行窗口
- 直接运行 mintty.exe 会先出现一个选择 shell 的窗口
    - 可以选择 msys 和 mingw
    - 对于 windows 而言 mintty 其实是一个 gui 进程
- 直接运行 /usr/bin/bash.exe 或 mintty.exe 会缺失很多环境变量，没有特别设置的情况下，基本不可用
- /git-bash.exe /git-cmd.exe /bin/bash.exe 这些被称为 git-rapper
    - 这是相关文档 https://gitforwindows.org/git-wrapper.html
...
powershell 版本 gitbash.ps1
```powershell
& "$PSScriptRoot\..\bin\bash.exe" -l @args
```
...
nc → Network Connections（网络连接）
pa → Panel Applet（控制面板小程序）
...
Program Files (x86) 会被缩短为 PROGRA~2
Common Files 会被缩短为 COMMON~1
...
这几种方式还可以顺利运行吗
    1 和 3 基本一致
        如果没有 shebang ，就会由 /bin/sh 解释执行
    2 的情况，不管有没有 shebang ，都是有 /bin/sh 解释执行
...
cmd /c dir 是执行完dir命令后关闭cmd。
cmd /k dir 是执行完dir命令后不关cmd。
cmd /c start dir 会打开一个新窗口后执行dir指令，原窗口会关闭。
cmd /k start dir 会打开一个新窗口后执行dir指令，原窗口不会关闭。

start dir 打开一个新窗口后执行dir指令，原窗口不会关闭，新窗口不会关闭。
start /b dir 执行dir指令，但不创建新窗口
start cmd "dir" // 这个是没有用的，只会打开一个新的窗口，因为只有 /c 和 /k 才会执行命令
这几句也是有效的
start cmd /k "dir"
start cmd /c "dir" 这个会一闪而过，因为 dir 命令执行完后，cmd也会关闭
start /b cmd /k "dir"
start /b cmd /c "dir"

这样能看到详细的帮助，在帮助里 cmd 被称为 Windows 命令解释器
cmd /?

start /?
启动一个单独的窗口以运行指定的程序或命令
看来 start 本质上也只是一个普通的命令

help
在 cmd 中直接运行 help 输出的帮助和 cmd /? 输出的不一样
    help cmd 和 cmd /? 输出是一样的
...
有什么区别？
    如果在命令行里是没有区别的
    加 call 是跨批处理文件调用的标准方式
        执行完 script.bat 后，控制权会返回到原批处理文件，继续执行后续命令
    不加 call（直接 script.bat）   
        执行完 script.bat 后，父脚本会直接退出，后续命令永远不会运行。  
        本质是父脚本将控制权完全交给 script.bat，且不再回收。
    为什么会出现这种行为？ 
        CMD 的批处理引擎默认将 script.bat 视为替换当前脚本的指令（类似跳转），而非“子调用”。  
        call 的作用是显式声明“这是一个子调用”，要求引擎保留父脚本的执行上下文。
