...
cron 在容器内运行 还是 在宿主机运行？
    在容器中运行
        supervisor
        https://github.com/aptible/supercronic/
        在容器中运行一个无限循环的 bash
        使用容器的 healthcheck
        在容器中运行 cron ，然后在前台运行 tail -f /var/log/cron.log
            https://blog.crackcreed.com/zai-dockerrong-qi-zhong-zhi-xing-ding-shi-ren-wu/
    在容器外运行
        cron 和 docker exec


为什么 定时任务 做不到毫秒级？
因为 linux 和 windows 都不是 实时系统
...
milli 来自拉丁语，只能作为单词前缀，千分之一的意思
mini  迷你，相对于同类更小
small 最宽泛、最通用的“小”，没有固定的参照标准，依靠语境和比较，描述任何事物的“小”

软实时 和 硬实时
    硬实时操作系统必须使任务在确定的时间内完成。
    软实时操作系统能让绝大多数任务在确定时间内完成。

普通的 linux 可以通过修改系统配置实现 软实时 抖动大概在 10毫秒
linux 可以通过加上实时内核补丁(PREEMPT_RT) 实现 高确定性的软实时和部分硬实时，抖动大概在 50微秒
windows iot 是可以实现 软实时 的


还有一种"坚实时/固实时"（Firm Real-time）
在学术和工程中，有时还会提到第三种类型：
    坚实时（Firm Real-time）：任务错过截止时间后结果无用，但不会造成灾难。
    例如：视频解码中某一帧超时，直接丢弃该帧，继续处理下一帧，不影响整体播放。

从定义上看，实时系统的速度不一定会快，，实时主要是明确执行任务时间的确定性
虽然没有明确的定义，硬实时系统的抖动一般会小于 10微秒
