...
### 编码/编码规则/encoding
编码就是字符所对应的二进制数字。例如，拉丁字母 `A` 在 ascii 里所对应的编码是 01000001 ，汉字 `一` 在 utf-8 里所对应的编码是 11100100 10111000 10000000 。
...
### 本地化时期
因为 ASCII 只支持英文，同时为了保证前向兼容，所以其它国家在 ASCII 的基础上作出各自的拓展。一般的拓展都是把 ASCII 中的最高位利用起来，ASCII 拓展的字符集中比较有影响的是 ISO 8859-1，这是拉丁字母的拓展，基本覆盖西欧各国的字母，所以也被称为 latin-1，这个字符集也是 JAVA 的默认字符集。ISO 8859 除了 lation-1 之外还有 14 个字符集，用来表示欧洲各个国家的文字。
...
### BIG5，BIG5-HKSCS
BIG5 (大五码) 是台湾人搞的中文字符集，比 GB2312 要早，收录的字数也比 GB2312 多，但没有简体字，在大陆这边几乎没用。BIG5-HKSCS (Hong Kong Supplementary Character Set, 香港增补字符集) 是香港人基于 BIG5 搞的一套字符集，就是在 BIG5 基础上加上一些粤语字和一部分简体字。
...
在 Windows 系统下，按住 alt ，然后键入 Unicode 编号，就能输入对应的字符。
...
### Code Point, Code Unit, Code Space
...
其余的 16 个平面称为 辅助平面或增补平面(Supplementary Plane, SP)。
...
代理区的范围是 0xD800-0xDFFF ，共 2048（256*8） 个码点。

代理机制，大概就是用两个代理区的码点来表示一个增补平面的字符。
两个码点就是 4 个字节。

代理区的码点在 usc-2 是有对应的字符。

通常如果一个程序声明支持 usc-2 ，其实就是暗示不支持增补平面的字符。
...
unicode 符号范围（十进制）|unicode 符号范围（16进制）|utf-8 编码方式
-|-|-
0 - 127|0x00 - 0x7f|0xxxxxxx
128 - 2047|0x80 - 0x7ff|110xxxxx 10xxxxxx
2048 - 65535|0x800 - 0xffff|1110xxxx 10xxxxxx 10xxxxxx
65536 - 1114111|0x10000 - 0x10ffff|11110xxx 10xxxxxx 10xxxxxx 10xxxxxx

下面是一个简单的，把 unicode 转换成 utf-8 的例子，但不考虑超过 4 字节的 utf-8 ，因为超过 4 字节的 utf-8 ，无符号的整型存不下来。
...
1. 加载字体
1. 输入字符的编码
1. 根据字体文件里的 cmap 把字符的编码转换成对应的字形索引
1. 根据索引从字体中加载这个字形
1. 把字形渲染成位图
