...
多字节编码，就是指一个编码里，单个字符的字节可能会多于两个。例如， utf-8 。在 utf-16 这类字符能多于两个字节的编码出现之前，双字节编码也会被称为多字节编码。
...
|0|是数字0，占四个字节，全部位数都是0|
|'0'|是字符0，占一个字节，对应的ascii码 48|
|'\0'|是字符串结束的字符，占一个字节，全部位数都是0|
