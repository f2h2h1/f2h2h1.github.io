...
echo "<?php echo php_sapi_name();echo PHP_EOL;" | php-cgi
echo "<?php echo php_sapi_name();echo PHP_EOL;" | php-cgi -d cgi.force_redirect=0
echo "<?php echo php_sapi_name();echo PHP_EOL;" > sapi.php


这一段是有效果的，但 sapi name 依然是 cgi-fcgi
export REQUEST_METHOD=GET
export SCRIPT_FILENAME=/c/Users/a/dev/stocks/sapi.php
export QUERY_STRING="a=1&b=2"
export CONTENT_LENGTH=0
export SERVER_PROTOCOL=HTTP/1.0
export SERVER_NAME=localhost
export SERVER_PORT=80
export DOCUMENT_ROOT=/c/Users/a/dev/stocks
php-cgi

这一段也是有效果的，删掉 PATH_TRANSLATED 依然有效，删掉 PATH_TRANSLATED 和 SCRIPT_NAME 依然有效
export REQUEST_METHOD=GET
export PATH_INFO=dev/stocks/sapi.php
export PATH_TRANSLATED=/c/Users/a/dev/stocks/sapi.php
export SCRIPT_NAME=/c/Users/a/dev/php-8.1.26-nts-Win32-vs16-x64/php
export QUERY_STRING="a=1&b=2"
export CONTENT_LENGTH=0
export SERVER_PROTOCOL=HTTP/1.0
export SERVER_NAME=localhost
export SERVER_PORT=80
export DOCUMENT_ROOT=/c/Users/a/
php-cgi
...
pci dss
    ...
    支付卡行业数据安全标准
    由信用卡品牌联合制定的安全标准，旨在保护持卡人数据，适用于所有处理、存储或传输信用卡信息的组织。
hipaa
    Health Insurance Portability and Accountability Act
    健康保险流通与责任法案
    美国法律，规定医疗机构和相关服务提供商必须保护患者的医疗信息隐私和安全。
nist
    National Institute of Standards and Technology
    美国国家标准与技术研究院
    美国联邦机构，负责制定和推广技术标准，包括网络安全框架（如 NIST CSF）和加密标准（如 AES）。
PQC
    Post-Quantum Cryptography
    后量子密码
    随着量子计算的发展，传统的RSA和ECC算法未来可能被攻破。
    为应对这一风险，业界正逐步过渡到后量子密码（PQC）。
    目前主流做法是采用混合密钥协商（HybridKEMs）：
        同时使用传统算法和后量子算法进行加密，以确保兼容性和安全性。
    自 Google Chrome 116（2023年8月） 起已默认支持混合PQC算法（如 X25519+Kyber），
    Firefox、Cloudflare 等也已开始部署。

除此之外，还要考虑各种浏览器，浏览器的各个版本，操作系统的兼容性
...
h2 约等于 二进制的http1.1
    http2虽然解决了HTTP队头阻塞的问题，但是对于TCP队头阻塞的问题却无能为力
h3 约等于 使用 udp 的 h2 并强制启用 tls
    使用了 quic 一个基于 udp 的协议替代 tcp
    quic 也可以用在其它协议上
    h3 使用 udp的443端口
    但 udp 很容易遇到 运营商Qos
...
HTTP2.0是SPDY的升级，它采用了SPDY很多的特性和技术
SPDY ，由Google开发，但 15 年时谷歌已经放弃这个，转向 http2
SPDY并不是一个标准协议，后来SPDY未能单独成为正式标准，不过SPDY开发组的成员全程参与了HTTP/2的制定过程。

NPN 和 ALPN 是 TLS的扩展协议
NPN是SPDY使用的扩展，而ALPN是HTTP2使用的扩展
NPN(Next Protocol Negotiation)
ALPN (Application Layer Protocol Negotiation) 


rfc7540
h2 -> HTTP/2 uses Transport Layer Security (TLS)
h2c -> HTTP/2 is run over cleartext TCP

h2连接方式：
    升级机制（Upgrade）：客户端首先发起 HTTP/1.1 请求，并在 Header 中包含 Upgrade: h2c 和 HTTP2-Settings。
        如果服务器支持，则返回 101 Switching Protocols 响应，随后切换为 HTTP/2 帧格式进行通信。
        类似于 websocket
    直接连接（Prior Knowledge）：客户端直接使用 HTTP/2 的魔法字符串（Magic String）和 SETTINGS 帧发起连接，省去 HTTP/1.1 升级的往返。

h2c是基于明文TCP传输的HTTP/2协议，无需TLS加密，旨在减少握手延迟，提升性能。
常用于内网、微服务组件通信，但主流的浏览器都不支持 h2c ，
curl是支持h2c的
