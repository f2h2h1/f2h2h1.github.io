...
使用 powershell 调用 BCryptGenRandom 生成随机数
```powershell
...
public class Bcrypt {
    // 定义 BCRYPT_USE_SYSTEM_PREFERRED_ALGORITHM 标志值为 2
    public const uint BCRYPT_USE_SYSTEM_PREFERRED_ALGORITHM = 2;

    [DllImport("Bcrypt.dll", CallingConvention = CallingConvention.StdCall)]
    public static extern uint BCryptGenRandom(IntPtr hAlgorithm, byte[] pbBuffer, uint cbBuffer, uint dwFlags);
}
'@

$bufferSize = 1 # 取值范围是 1 2 4 8 16
# 生成 bufferSize 字节的随机数
$buffer = New-Object byte[] $bufferSize

# 调用 BCryptGenRandom
$result = [Bcrypt]::BCryptGenRandom([IntPtr]::Zero, $buffer, $bufferSize, [Bcrypt]::BCRYPT_USE_SYSTEM_PREFERRED_ALGORITHM)

# 检查返回值是否为 0（STATUS_SUCCESS）
if ($result -eq 0) {
    if ($bufferSize -eq 1) {
        $randomNumber = $buffer
    } elseif ($bufferSize -eq 2) {
        $randomNumber = [BitConverter]::ToUInt16($buffer, 0)
    } elseif ($bufferSize -eq 4) {
        $randomNumber = [BitConverter]::ToUInt32($buffer, 0)
    } elseif ($bufferSize -eq 8) {
        $randomNumber = [BitConverter]::ToUInt64($buffer, 0)
    } elseif ($bufferSize -eq 16) { # ToUInt128 在 .NET 9 之后的版本才可以用
        $randomNumber = [BitConverter]::ToUInt128($buffer, 0)
    }
    Write-Output "生成的随机数为: $randomNumber"
} else {
    Write-Error "调用 BCryptGenRandom 失败，错误代码: $result"
}
```
...
CWE 是一项社区驱动的计划，旨在提供常见软件弱点、漏洞和编码错误的标准化且全面的列表。
CWE 由 Mitre Corporation 开发和维护，是网络安全专业人员和软件开发人员的宝贵资源。
...
Bouncy Castle 是用于在计算机程序中实现加密的 API 集合。
它包括用于 Java 和 C# 编程语言的 API。
这些 API 由澳大利亚注册的慈善组织 Legion of the Bouncy Castle Inc. 提供支持。
Bouncy Castle 起源于澳大利亚，因此美国对从美国出口密码学的限制不适用于它。
...
Feistel 密码结构
国密
线性同余方法（Linear Congruence Generator , LCG）
Web Crypto API
...
证书透明度（Certificate Transparency，CT）
    Merkle 树 梅克尔树或默克尔树
        是 哈希数 也是 二叉树
    证书签署时间戳 signed certificate timestamp (SCT)
字体加密
...
Stunnel
    ...
    配置
        服务端
            创建 psk.txt 的文件， password 可以是很长的字符串，password 长度要大于 16
                user1:password1
                user2:password2
            创建 stunnel.conf 文件
                ; 指定 ssl 版本
    ...
    例子
        ...
        stunnel 和代理服务配合使用
            0. 假设代理服务的连接地址是 socks5://127.0.0.1:1080
            ...
            验证代理是否有生效
            curl -i -v -L --proxy socks5://127.0.0.1:1080 https://www.baidu.com

            关注一开始是否有 socks5 代理的输出
            关注 tls 的版本和密码套件
            SSL connection using TLSv1.2 / ECDHE-RSA-AES128-GCM-SHA256

            使用 psk 应该是可以的。。。再试一次
            psk 确实是有效的，但似乎不能用 tls1.3

            虽然能成功连接，但没有客户端验证
            客户端验证似乎还需要更复杂的配置

ciphers 是设置 tls1.2及以下版本的
ciphersuites 这个才是设置 tls1.3 的

其实 stunnel 能直接支持 socks5 代理，不需要额外的代理软件
stunnel 的 socks5 代理
服务端设置
output=/var/log/stunnel.log ; 要指定日志输出的位置，不然日志会直接输出到当前终端里
syslog=no ; 不使用 syslog 记录日志
[proxy]
debug=4 ; 降低调试级别，减少日志
ciphers = PSK
PSKsecrets = psk.txt
...
一些配置是 unix only ，一些配置是 win32 only ，所以还是要认真看一下文档
https://www.stunnel.org/static/stunnel.html

stunnel 虽然以 GPL2 开源，但似乎不能免费地商用

使用 apache2 开源的 tls代理
https://github.com/ghostunnel/ghostunnel
可以从 releases 中下载已经编译好的二进制文件
...
TLS 是应用程序中立的;用 RFC 5246 的话来说：
TLS 的一个优点是它与应用程序协议无关。
更高级别的协议可以透明地叠加在 TLS 协议之上。
但是，TLS 标准并未指定协议如何使用 TLS 添加安全性;
关于如何启动 TLS 握手以及如何解释交换的身份验证证书的决定，留给在 TLS 上运行的协议的设计者和实施者的判断。
...
带有关联数据的身份验证加密
AEAD加密模式是一种结合了加密和认证的加密技术。
它允许在加密数据的同时，对一些附加的非加密数据（称为关联数据）进行认证。
这种模式特别适用于需要同时保护数据机密性和完整性的场景，如网络通信、数据存储等。
...
混淆（confusion）与扩散（diffusion）是密码学算法设计（尤其是对称密码）的原则，
由克劳德·香农（Claude Shannon）在1949年的论文《保密系统的通信理论》中首次系统化提出。
它们共同作用，确保加密算法的安全性
混淆和扩散是设计原则，
而替换（Substitution）、排列（Permutation）、置换（Transposition）和移项（Shift）是实现这些原则的具体操作。

混淆（Confusion）
    定义：
        混淆是一种设计原则，旨在使密钥与密文之间的关系尽可能复杂和难以预测。
        其核心是隐藏密钥的统计特性，防止攻击者通过分析密文（如频率分析或差分分析）推导出密钥。
    目的：
        增强算法的抗攻击性，特别是抵抗已知明文攻击、选择明文攻击和统计分析。
        确保密文不泄露任何关于密钥的线索，即使攻击者拥有大量明文-密文对。
    机制：
        通常通过非线性操作实现，例如替换盒（S-box，Substitution Box）。
        非线性操作确保密钥比特和密文比特之间没有简单的数学关系（如线性或多项式关系），从而增加破解难度。

扩散（Diffusion）
    定义：
        扩散是一种设计原则，旨在使明文的单个比特变化影响密文中的多个比特，从而隐藏明文的统计特性（如冗余、模式或频率分布）。
        其核心是“雪崩效应”（Avalanche Effect）：明文的微小变化（如翻转一个比特）应导致密文发生巨大且不可预测的变化（通常50%的比特改变）。
    目的：
        消除明文的局部相关性，防止攻击者利用明文的统计模式（如英文文本中字母“e”的高频出现）进行攻击。
        确保密文看起来是随机的，即使明文有规律。
    机制：
        通常通过线性操作实现，例如排列（Permutation）、置换（Transposition）或线性变换（如矩阵乘法）。
        这些操作分散明文比特的影响，使每个明文比特“扩散”到多个密文比特。

替换（Substitution）
    将明文元素（如比特、字节或字符）映射为不同的密文元素，而不改变其位置。映射规则通常由密钥控制
排列（Permutation）
    重新排列元素（如比特、字节或字符）的顺序，而不改变元素本身的值
置换（Transposition）
    在古典密码学中特指字符或符号的位置重新排列。它与排列（Permutation）高度相似，但传统上用于描述字符级（而非比特级）操作
    置换本质上是排列的一种形式，但在术语使用上：
        “排列（Permutation）”更通用，适用于比特、字节或字符，常见于现代密码。
        “置换（Transposition）”特指古典密码中的字符重排。
移项（Shift）
    将元素在序列中平移固定位置
    移位是替换的特例（当应用于字符值）或排列的特例（当应用于位置）。
        字符移位（如凯撒）→ 替换。
        位置移位（如AES ShiftRows）→ 排列。
...
sni (Server Name Indication)
    sni 是 tls client hello 的一部分
    会明文显示 域名
esni (Encrypted Server Name Indication)
    在 sni 的基础上加密域名，但特征会很明显，因为 sni 的域名被加密了
ech (Encrypted Client Hello)
    加密整个 client hello

ensi 和 ech 加密用的 公钥 是通过 dns 查询对应的域名记录获取的
所以还需要这几个协议
DoH 和 DoT 以及 DNSSEC
才可以保证全程加密

即使全程加密 依然有可能被 主动探测/流量指纹/DPI（深度包检测） 发现
所以 混肴 和 伪装 依然是必要的
