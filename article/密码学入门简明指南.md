...
- 导出公钥
```
gpg --armor --output public-key.txt --export "sdk64 <sdk64@qq.com>"
```

- 导出私钥 需要输入密码
```
gpg --armor --output private-key.txt --export-secret-keys "sdk64 <sdk64@qq.com>"
```
...
- 加密文件 生成二进制文件
gpg --recipient "sdk64 <sdk64@qq.com>" --output demo.en.txt --encrypt demo.txt
加密文件 生成 ASCII 码的文本文件
gpg --armor --recipient "sdk64 <sdk64@qq.com>" --output demo.en.txt --encrypt demo.txt

- 解密文件 需要输入密码
gpg --recipient "sdk64 <sdk64@qq.com>" --output demo.de.txt --decrypt demo.en.txt

对文件进行签名 运行上面的命令后，当前目录下生成demo.txt.gpg文件，这就是签名后的文件。这个文件默认采用二进制储存
gpg --recipient "sdk64 <sdk64@qq.com>" --sign demo.txt
生成 ASCII 码的签名文件 当前目录下生成demo.txt.asc文件，后缀名asc表示该文件是ASCII码形式的
gpg --armor --recipient "sdk64 <sdk64@qq.com>" --sign demo.txt

--sign 会把内容和签名合并在一个文件里
如果想生成单独的签名文件，与文件内容分开存放，可以使用detach-sign参数。
gpg --recipient "sdk64 <sdk64@qq.com>" --detach-sign demo.txt
同样地，生成的是二进制的文件，如果要生成 ascii 码的文本文件需要加上 --armor 参数
-->
