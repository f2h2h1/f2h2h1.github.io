...
b=$(for i in $(seq 1 $n); do echo "curl -o /dev/null -s -w %{http_code}::%{time_namelookup}::%{time_connect}::%{time_starttransfer}::%{time_total}::%{speed_download}; $TEST_URL ";echo ","; done)
...
# echo $b;
c=$(echo $b | xargs -d "," -n 1 -P $c bash -c '$0')
# echo $c;
...
- 一共请求 100 次
- 至多同时运行两次请求
- 输出各项数据的，总计，平均数，最大值，最小值，中位数，标准差
