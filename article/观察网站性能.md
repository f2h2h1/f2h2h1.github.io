...
echo $(for i in {1..10}; do echo "curl -o /dev/null -s -w %{time_namelookup}::%{time_connect}::%{time_starttransfer}::%{time_total}::%{speed_download}\n www.baidu.com";echo ";"; done) | xargs -d ";" -n 1 -P 4 bash -c '$0'

a=20;echo $(for i in $(seq 1 $a); do echo "curl -o /dev/null -s -w %{time_namelookup}::%{time_connect}::%{time_starttransfer}::%{time_total}::%{speed_download}\n www.baidu.com";echo ";"; done) | xargs -d ";" -n 1 -P 4 bash -c '$0'


这个命令能输出当前系统 cpu 的核心数
nproc

nproc | awk '{printf ("%d", $1*2)}'
nproc | awk '{printf ("%d", $1*2)}' | awk '{printf ("%d", $1*100)}'
