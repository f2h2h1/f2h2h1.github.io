...
- 服务器推: 旧时代的前端里，把服务器主动发送消息给浏览器的技术称为 服务器推 。因为没有 sse 和 websocket ，旧时代的前端要实现服务器推其实是挺复杂的，大多数情况下会借助各种插件(Flash, Java Applet, Silverlight, ActiveX)。这里提及的服务器推和 HTTP/2 的 server push 是不一样的
