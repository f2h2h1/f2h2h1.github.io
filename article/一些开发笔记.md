...
在 github releases 下载二进制文件后，复制到 git for windows 的 path 目录
```
curl -L -o <GIT>/mingw64/bin/jq.exe https://github.com/jqlang/jq/releases/download/latest/jq-windows-amd64.exe
curl -L -o <GIT>/usr/bin/jq.exe https://github.com/jqlang/jq/releases/download/latest/jq-windows-amd64.exe
```

jq-windows-amd64.exe 和 jq-win64.exe 是一样的，只是命名方式不同
